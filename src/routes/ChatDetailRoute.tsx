...
export function ChatDetailRoute() {
  ...
  const getRequestedModelForConversation = useConversationModelStore(
    (state) => state.getRequestedModelForConversation
  );
    ...
    const loadConversation = async () => {
      ...
      try {
        ...
        if (!loadedConversation) {
          const fallbackModelId = SMART_MODEL.id;
          const nextConversation = createConversation(fallbackModelId, id);
          setConversation(nextConversation);
          await hydrateConversation(nextConversation.id, nextConversation.model);
          return;
        }

        setConversation(loadedConversation);
        await hydrateConversation(loadedConversation.id, loadedConversation.model);
      } catch (error) {
  ...
  const requestedModelId = getRequestedModelForConversation(conversation.id);
  const runtimeModelId = currentModel?.id ?? requestedModelId;
