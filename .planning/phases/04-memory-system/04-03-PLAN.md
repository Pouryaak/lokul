  ...
  <action>
    ...
    2. Create src/hooks/useMemory.ts:
       ...
       const clearAllFacts = async () => {
        // Back up all facts for undo
        const allFacts = await getMemoryFacts();
        if (allFacts.kind === "ok") {
          clearedFactsRef.current = allFacts.value;
        }

        // Visual clear immediately — optimistic update
        // DO NOT call clearAllMemory() here yet
        setOptimisticClear(true); // local state to hide facts from UI

        // Schedule actual IndexedDB wipe after 8 seconds
        const wipeTimer = window.setTimeout(async () => {
          await clearAllMemory();
          clearedFactsRef.current = [];
