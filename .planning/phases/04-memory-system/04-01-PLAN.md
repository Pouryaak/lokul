...
<task type="auto">
  ...
  <action>
    ...
    4. Implement these helper functions:
       ```typescript
       // String-level deduplication only (catches identical re-extractions)
        // Semantic duplicate detection is handled by the LLM via updates_previous flag
        function isExactDuplicate(fact1: string, fact2: string): boolean
        // Semantic duplicates are caught by the extraction prompt returning updates_previous: true, which routes through resolveConflictingFact not mergeDuplicateFact
...
<task type="auto">
  ...
  <action>
    1. Create src/lib/memory/extraction.ts:
      ...
      - response_format is not universally supported in WebLLM
      - Rely on prompt engineering instead — add "Respond ONLY with JSON"
      - to the system prompt and wrap JSON.parse in try/catch
      - Do NOT pass response_format to the engine call
